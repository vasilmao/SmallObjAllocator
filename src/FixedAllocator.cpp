...
FixedAllocator::FixedAllocator(size_t block_size) : block_size_(block_size) {
    chunks_.emplace_back(block_size, kChunkSize);
}
...
void* FixedAllocator::Allocate() {
    if (alloc_chunk_index_ == (size_t)-1 || !chunks_[alloc_chunk_index_].HasFreeStorage()) {
        ...
        if (i == chunks_.size()) {
            // we need new chunk
            chunks_.emplace_back(block_size_, kChunkSize);
            alloc_chunk_index_ = i;
            dealloc_chunk_index_ = i;
        }
    }
    assert(alloc_chunk_index_ != (size_t)-1);
...
void FixedAllocator::Deallocate(void* ptr) {
    ...
    if (chunks_[dealloc_chunk_index_].ContainsMemory(ptr, block_size_, kChunkSize)) {
        block_found = true;
    } else {
        for (size_t i = 1; dealloc_chunk_index_ - i >= 0 || dealloc_chunk_index_ + i < chunks_.size(); ++i) {
            ...
            if (low_index >= 0) {
                if (chunks_[low_index].ContainsMemory(ptr, block_size_, kChunkSize)) {
            ...
            if (high_index < chunks_.size()) {
                if (chunks_[high_index].ContainsMemory(ptr, block_size_, kChunkSize)) {
